 ...
 * 
 */
static void freeBagNode(Node current);
/*-----------------------------------------------------------*/
/*
 *  newBag()
 *
 ...
 */
Bag newBag() {
    Bag newBag = emalloc(sizeof(struct bag));
    newBag->first = NULL;
    newBag->size = 0;
    return newBag;
}
...
void add(Bag bag, const void *item, size_t nItem) {
    Node saveFirst = bag->first;
    Node newNode = malloc(sizeof(struct node));
    newNode->item = malloc(nItem);
    memcpy((void*) newNode->item, item, nItem);
    newNode->next = saveFirst;
    newNode->size = nItem;
    bag->first = newNode;    
    bag->size++;
}    
...
void *itens(Bag bag, Bool init) {
    ...
    void* temp;
    if(init == TRUE) {
        temp = malloc(bag->first->size);
        memcpy(temp, bag->first->item, bag->first->size);
        bag->lastAccess = bag->first;
    }
    else {
        if(bag->lastAccess == NULL || bag->lastAccess->next == NULL) {
            return NULL;
        }
        temp = malloc(bag->lastAccess->next->size);
        memcpy(temp, bag->lastAccess->next->item, bag->lastAccess->next->size);
        bag->lastAccess = bag->lastAccess->next;
    }
