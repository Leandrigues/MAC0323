...
public class MeuTST<Value extends Comparable<Value>> {
    ...
    private class Item implements Comparable<Item> {
        ...
        public Item(String key, Value val) {
            this.key = key;
            this.val = val;
        }

        public int compareTo(Item compare) {
            if (this.val.compareTo(compare.val) > 0)
                return 1;
            else if (this.val.compareTo(compare.val) < 0)
                return -1;
            return 0;
        }
    }
    ...
    public Iterable<String> keysWithPrefixByValue(String prefix) {
        ...
        MaxPQ<Item> maxQ = new MaxPQ<Item>();
        ...
        if (x.val != null) {
            item = new Item(prefix, x.val);
            maxQ.insert(item);
        } 

        collect2(x.mid, new StringBuilder(prefix), maxQ);
    ...
    private void collect2(Node<Value> x, StringBuilder prefix, MaxPQ<Item> queue) {
        if (x == null) 
            return;
        collect2(x.left,  prefix, queue);
        
        String key = prefix.toString() + x.c;
        Item item = new Item(key, x.val);

        if (x.val != null) 
            queue.insert(item);

        collect2(x.mid, prefix.append(x.c), queue);
        prefix.deleteCharAt(prefix.length() - 1);
        collect2(x.right, prefix, queue);
    }
    ...
    public Node<Value> delete(Node<Value> x, String key, int d) {
        if (x == null)
            return null;
        ...
        if(x.mid != null || x.right != null || x.left != null || x.val != null)
            return x;
    ...
    public static void main(String[] args) {
        ...
        StdOut.println("Estado inicial da tabela:\n");
        for(String s : st.keys()) {
            StdOut.print(st.get(s) + ": ");
            StdOut.println(s);
        }

        Iterable<String> q = st.keysWithPrefixByValue("Ca");
        
        StdOut.println("\nStrings com prefixo 'Ca' em ordem descrescente de valor\n");
        
        for(String t : q) {
            StdOut.print(st.get(t) + ": ");
            StdOut.println(t);

        }

        StdOut.println("\nDeletando todas com prefixo 'Ca'\n");
